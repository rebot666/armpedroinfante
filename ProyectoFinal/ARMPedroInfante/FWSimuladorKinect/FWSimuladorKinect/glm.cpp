...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    for (i = 0; i < nummaterials; i++) {
        ...
        // Default initialization for additional properties
		model->materials[i].textureid = 0;
		model->materials[i].wrapmode = GL_REPEAT;
		model->materials[i].alpha = 1.0;
		model->materials[i].stexdis=0.0;
		model->materials[i].ttexdis=0.0;
		model->materials[i].stexspeed=0.0;
		model->materials[i].ttexspeed=0.0;
		model->materials[i].scale=1.0;
		model->materials[i].lighting=true;
		model->materials[i].vertexshader = 0;
		model->materials[i].fragmentshader = 0;
    }
    ...
    /* Muestra los nombres de los materiales usados por el modelo */
	printf("Materiales usados en el modelo:%s\n",model->pathname);
	for (i = 0; i < model->nummaterials; i++) {
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    while (group) {
		...
		if(material->alpha < 1.0) //Enables blending
		{
			glEnable(GL_BLEND);
			glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
			// Changes alpha of diffuse component of material
            material->diffuse[3]=material->alpha;
		}

		if((mode & GLM_TEXTURE) && (material->textureid > 0)) //Enables texturing
		{          
		  ...
		  // Changes diffuse component of material to use texture colors
		...
		// Restores from texture transformations
		if((mode & GLM_TEXTURE) && (material->textureid > 0) && ((material->scale!=1.0) || (material->stexspeed!=0.0) || (material->ttexspeed!=0.0)))
		  {
			  glPopMatrix();
